...
def structured_mock():
    ...
    s.delete_preference = AsyncMock(
        return_value={"key": "x", "value": "y", "confidence": 1.0, "inferred": False, "source": "extraction"}
    )
...
class TestParseOps:
    ...
    def test_parse_rejects_unknown_table(self):
        with pytest.raises(OpValidationError):
            _parse_one({"op": "ADD", "target": {"table": "unknown", "key": {}}, "payload": {}, "reason": "x"})

    def test_parse_rejects_delete_on_persons(self):
        with pytest.raises(OpValidationError, match="DELETE on persons"):
            _parse_one({"op": "DELETE", "target": {"table": "persons", "key": {"name": "X"}}, "reason": "remove"})
    ...
    def test_parse_rejects_missing_reason_for_non_noop(self):
        with pytest.raises(OpValidationError, match="reason required"):
            _parse_one(
                {
                    "op": "ADD",
                    "target": {"table": "preferences", "key": {"person": "A", "key": "b"}},
                    "payload": {"value": "c"},
                }
            )
    ...
    def test_parse_rejects_events_update(self):
        with pytest.raises(OpValidationError, match="events table only supports ADD"):
            _parse_one(
                {
                    "op": "UPDATE",
                    "target": {"table": "events", "key": {"event_type": "correction"}},
    ...
    def test_idempotency_hash_is_stable(self):
        op1 = MemoryOp(
            op="ADD", target_table="preferences", target_key={"person": "A", "key": "b"}, payload={}, reason="r"
        )
        op2 = MemoryOp(
            op="ADD",
                ...
                "ops": [
                    {
                        ...
                        "payload": {"content": "two lines"},
                        "reason": "add member",
                    }
                ]
            }
        )
        result = await applier.apply_all(ops, "Alice", "sess-1", memory_snapshot={"family": "one line"})
        ...
        ops = parse_ops_json(
            {
                "ops": [
                    {
                        "op": "ADD",
                        "target": {"table": "preferences", "key": {"person": "A", "key": "k1"}},
                        "payload": {"value": "v"},
                        "reason": "r",
                    },
                    {
                        "op": "ADD",
                        "target": {"table": "preferences", "key": {"person": "A", "key": "k2"}},
                        "payload": {"value": "v"},
                        "reason": "r",
                    },
                ]
...
class TestBeforeState:
    ...
    async def test_before_state_captured_from_snapshot_for_memory_entries(self, applier, pool_mock):
        ...
        await applier.apply_all(ops, "Alice", "sess-1", memory_snapshot={"family": "OLD CONTENT"})
...
# -------------------------------------------------------------------------
# B2 (JANE-81): recently_removed guard + on_pref_add callback
# -------------------------------------------------------------------------


class TestRecentlyRemovedGuard:
    @pytest.mark.asyncio
    async def test_apply_skips_add_when_check_returns_true(self, backend_mock, structured_mock, pool_mock):
        """ADD on preferences gets downgraded to NOOP when guard fires."""
        check = AsyncMock(return_value=True)
        applier = OpApplier(
            backend=backend_mock,
            structured=structured_mock,
            pg_pool=pool_mock,
            recently_removed_check=check,
        )
        ops = parse_ops_json(
            {
                "ops": [
        ...
        result = await applier.apply_all(ops, "Alice", "sess-rrg")

        # Save was NOT called — op was downgraded to NOOP before dispatch.
        structured_mock.save_preference.assert_not_called()
        # Counters reflect a NOOP (one nooped, zero added).
        assert result.nooped == 1
        assert result.added == 0
        # The audit row has op='NOOP' with reason='recently_removed_guard'.
        insert_args = pool_mock._conn.execute.await_args_list[0].args
        assert insert_args[1] == "NOOP"
        assert insert_args[6] == "recently_removed_guard"
        # The check itself was awaited with the canonical pair.
        check.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_apply_invokes_on_pref_add_only_for_add_ops(self, backend_mock, structured_mock, pool_mock):
        """on_pref_add fires once per ADD on preferences, never on UPDATE/DELETE/NOOP."""
        on_add = AsyncMock()
        applier = OpApplier(
            backend=backend_mock,
            structured=structured_mock,
        ...
        ops = parse_ops_json(
            {
                "ops": [
                    ...
                    {
                        "op": "ADD",
                        "target": {"table": "preferences", "key": {"person": "Bob", "key": "lights"}},
                        "payload": {"value": "off"},
                        "reason": "r",
                    },
                ]
        ...
        await applier.apply_all(ops, "Alice", "sess-onadd")

        assert on_add.await_count == 2  # two ADDs

    @pytest.mark.asyncio
    async def test_apply_works_with_default_none_callbacks(self, applier, structured_mock):
        """Existing callers (no callables passed) still work unchanged."""
        ops = parse_ops_json(
            {
                "ops": [
                    {
        ...
        result = await applier.apply_all(ops, "Alice", "sess-default")
