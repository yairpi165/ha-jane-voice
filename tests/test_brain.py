...
from jane_conversation.brain import _classify_request, _build_context, _extract_text
...
class TestClassifyRequest:
    """Test _classify_request categorizes Hebrew input correctly."""

    # Commands
    def test_turn_on_light(self):
        assert _classify_request("תדליק אור") == "command"

    def test_turn_off_ac(self):
        assert _classify_request("תכבה מזגן") == "command"

    def test_open_shutter(self):
        assert _classify_request("תפתח תריס") == "command"

    def test_close_shutter(self):
        assert _classify_request("תסגור תריס") == "command"

    def test_boil_water(self):
        assert _classify_request("תרתיח מים") == "command"
    ...
    def test_good_morning_is_command(self):
        assert _classify_request("בוקר טוב") == "command"

    def test_good_evening_is_command(self):
        assert _classify_request("ערב טוב") == "command"

    def test_goodnight_with_name(self):
        assert _classify_request("לילה טוב ג'יין") == "command"
    ...
    def test_hi(self):
        assert _classify_request("היי") == "chat"

    def test_thanks(self):
        assert _classify_request("תודה") == "chat"

    def test_whats_up(self):
        assert _classify_request("מה קורה") == "chat"

    # Complex
    def test_create_automation(self):
        assert _classify_request("תיצרי אוטומציה שמדליקה אור בשש") == "complex"

    def test_explain(self):
        assert _classify_request("למה המזגן לא נכבה?") == "complex"

    def test_history_question(self):
        assert _classify_request("מתי המזגן נדלק לאחרונה?") == "complex"

    def test_shopping_list(self):
        assert _classify_request("תוסיפי חלב לרשימת קניות") == "complex"

    def test_send_message(self):
        assert _classify_request("תשלחי הודעה ליאיר") == "complex"

    # Default → complex
    def test_unknown_defaults_to_complex(self):
        assert _classify_request("מה הטמפרטורה?") == "complex"
...
class TestBuildContext:
    """Test _build_context generates concise home awareness."""

    @pytest.mark.asyncio
    async def test_includes_weather(self, hass_mock):
        context = await _build_context(hass_mock)
